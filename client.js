// um-dsh-azimg · Client 半（cordis_define 的 code.client 函数体）
...
//   L1 libraries     —— design-token stylesheet + i18n dicts + pure utils (no ctx dependency)
//   L2 feature 组件  —— 单一职责展示组件（卡片 / 模型行 / 状态行）
//   L3 app           —— SettingsPanel 状态编排 + Slot 装配与 Fiber 副作用回收

// ════════════════════════ L1 libraries (stylesheet + i18n dicts + pure utils) ════════════════════════
  ...
  apply(ctx) {
    ...
    function SettingsPanel() {
      ...
      function load(refresh) {
        ...
        host.call('state', { refresh: !!refresh }).then(function (data) {
          applySnapshot(data)
          setBusy(false)
      ...
      function choose(provider, model) {
        setBusy(true)
        setProbe(null)
        host.call('select', provider ? { provider: provider, model: model } : null).then(function (data) {
          setBusy(false)
      ...
      function runProbe() {
        ...
        host.call('probe', { provider: selection.provider, model: selection.model }).then(function (data) {
          setBusy(false)
          setProbe(data || null)
          if (data && data.ok) {
