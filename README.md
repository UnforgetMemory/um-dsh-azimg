...
| 文件 | 内容 |
|------|------|
| `host.js` | Host 半：工具注册 + 模型能力枚举 + 热切换状态 + RPC |
| `client.js` | Client 半：设置页 + 工具调用卡片 |
| `hero.png` / `hero.jpg` | 头图原始稿 / Web 优化版（README 引用后者） |

## 📋 版本

当前 **0.1.1** —— 变更历史见 [CHANGELOG.md](./CHANGELOG.md)。
...
- 如需长期安装（不依赖会话内 `cordis_define`），可改造为 `cordis.yml` 静态插件或 bundle 包
