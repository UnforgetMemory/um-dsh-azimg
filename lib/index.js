// um-dsh-azimg · 静态 Host 半（profile bundle 行挂载，ESM）
// 工具 um_analyze_img：多图 + 格式嗅探 + 限额预检 + 候选失败切换 + 结构化呈现元数据
// 设置数据服务 umimg：模型能力识别（image/text/unknown）+ 热切换选择 + 可用性探测
//   —— TypertRemoteService + Remote 标记（SRC 回退路径自动暴露 <ns>/<method>，零生成清单）
//
// 分层（依赖方向自上而下、禁止反向）：
...
//   L4 app       —— 装配：裸 ToolDefinition 注册 + Remote 设置服务挂载
//
// 注意：SRC 回退靠 Function.prototype.toString 解析参数名——本文件禁止压缩/混淆。
...
// ════════════════════════ L4 装配前的工具定义（裸 ToolDefinition，已编译 JSON Schema）════════════════════════
...
const TOOL = {
  ...
  // execute 在 apply 内绑定（闭包 ctx）；见下 L4
  execute: null,
}

export const name = 'um-dsh-azimg'

export const inject = ['tools', 'subagents', 'llm', 'agents', 'attachments', 'fs']

export function apply(ctx) {
  // ══ store：会话内存态（动态/静态均不持久化）══
  ...
  // ════════════════════════ L4 app（装配：工具 + Remote 设置服务）════════════════════════

  TOOL.execute = async function execute(args, exec) {
  ...
  // ── 设置数据服务：umimg/state · umimg/select · umimg/probe（Client 经 /api 网关调用）──
  class UmimgSettingsService extends TypertRemoteService {
    constructor(ownerCtx) {
      super(ownerCtx, 'umimg')
      for (const init of umimgInits) init.call(this)
    }

    async state(request) {
      const force = !!(request && request.refresh)
    ...
    async select(request) {
      if (!request || typeof request.provider !== 'string' || typeof request.model !== 'string' ||
          request.provider === '' || request.model === '') {
        selection = null
        return { selection: selection, warning: '' }
      }
      const next = { provider: request.provider, model: request.model }
      selection = next
      const snapshot = await enumerateModels(false)
    ...
    async probe(request) {
      return probeModel(request && request.provider, request && request.model)
    }
  }

  const umimgInits = []
  for (const method of ['state', 'select', 'probe']) {
    Remote(method)(UmimgSettingsService.prototype, {
      kind: 'method',
      name: method,
      static: false,
      private: false,
      addInitializer: function (init) { umimgInits.push(init) },
    })
  }

  ctx.plugin(UmimgSettingsService)
}
